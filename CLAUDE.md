...
- Programmatic edits (MCP edit tools, `ApplyEditsCommand`, `WriteFileCommand`, `FindReplaceFileCommand`, `DeleteLinesCommand`, `WriteBinaryFileCommand`) write straight to disk. If the document is open, the buffer reloads from disk via `editor.setValue`, which wipes Monaco's undo history. The agent's edit cannot be reverted with Ctrl+Z; users who need pre-edit content rely on source control or copies. There is no editor-routed code path for programmatic edits.
