...
export class ViewModeController {
    ...
    constructor({ splitRoot, editorPane, previewPane, onLayoutChanged, onModeChanged }) {
    ...
    setFlexShare(share) {
        ...
        // Only apply inline flex styles when Split is the active mode. In
        // Source/Preview the panes inherit the full-width CSS from
        // #split-root.mode-*; applying inline flex here would override the
        // CSS and pin a partial size on the visible pane. The share is still
        // stored internally so setMode(Split) can apply it later.
        if (this.#currentMode === ViewMode.Split) {
            this.#applyFlexShare();
        }
        this.#onLayoutChanged();
    }
