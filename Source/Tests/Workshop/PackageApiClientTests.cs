...
namespace Celbridge.Tests.Workshop;
...
public class PackageApiClientTests
{
    ...
    public void Setup()
    {
        ...
        _client = new PackageApiClient(_settingsService, _messageHandler);
    }
    ...
    public async Task ListPackages_SendsApiKeyHeaderAndParsesSummaries()
    {
        ...
        packages[0].WorkshopVersionCount.Should().Be(3);
        packages[0].LatestWorkshopVersion.Should().NotBeNull();
        packages[0].LatestWorkshopVersion!.WorkshopVersion.Should().Be(3);
        packages[0].LatestWorkshopVersion!.Author.Should().Be("alice");
        packages[1].Name.Should().Be("empty-package");
        packages[1].LatestWorkshopVersion.Should().BeNull();
    }
    ...
    public async Task GetPackage_ParsesVersionsAndAliases()
    {
        ...
        details.WorkshopVersions.Should().HaveCount(2);
        details.WorkshopVersions[0].WorkshopVersion.Should().Be(1);
        details.WorkshopVersions[0].Deleted.Should().BeTrue();
        details.WorkshopVersions[0].Summary.Should().Be("Initial release");
    ...
    public async Task PublishVersion_SendsMultipartAndParsesReceipt()
    {
        ...
        receipt.WorkshopVersion.Should().Be(4);
