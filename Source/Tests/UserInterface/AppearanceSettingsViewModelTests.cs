...
/// <summary>
/// Unit tests for the Appearance settings section view model. The stored theme is read through the real
/// SettingsService over an in-memory settings store fake. Selecting a theme dispatches ISetThemeCommand rather than writing
/// the setting here, so that is asserted against a substitute command service. The dispatch goes through
/// ExecuteImmediate because the settings dialog holds the command queue while it is open. A real
/// MessengerService carries the theme-changed broadcast the section follows.
/// </summary>
[TestFixture]
public class AppearanceSettingsViewModelTests
{
    ...
    private AppearanceSettingsViewModel _viewModel = null!;
    ...
    public void Setup()
    {
        ...
        _viewModel = new AppearanceSettingsViewModel(
            new NullLogger<AppearanceSettingsViewModel>(),
            _settingsService,
            stringLocalizer,
            _commandService,
            _messengerService);
    ...
    {
        // The View menu offers the same themes, so a change made there while this section is showing has
        // to be reflected here rather than leaving a stale selection.
        _viewModel.OnShown();
    ...
    [Test]
    public void AfterHiding_ThemeChangesAreNoLongerFollowed()
    {
        _viewModel.OnShown();
        _viewModel.OnHidden();
