...
/// <summary>
/// A hosted web surface's complete focus contract, supplied once at registration. The registry converges every
/// focus-gain signal for the surface (managed GotFocus, the JS DOM-focus bridge, and the macOS native click
/// monitor) onto a single report of its Panel, EditTarget, and ReleaseFocus. EditTarget is null for a surface
...
/// are not integrated. One registration per surface replaces the per-view GotFocus, native-monitor, JS-bridge,
/// and grant wiring: the registry owns those signals and reports each surface's focus to the focus service.
/// </summary>
public interface IWebViewFocusRegistry
{
    ...
    /// <summary>
    /// Reports focus for a surface whose JS client signalled DOM focus over the host bridge. Must be called on
    /// the UI thread. Ignored when the surface is not registered.
    /// </summary>
    void ReportFocus(CoreWebView2 coreWebView);
