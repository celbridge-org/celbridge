...
/// <summary>
/// Serves project files over HTTP on localhost via the broker's Kestrel server.
/// Files are served at /local/{resourceKey} and the file provider is swapped
/// when projects are loaded and unloaded.
/// </summary>
public class ProjectFileServer : IProjectFileServer, IDisposable
{
    private readonly ILogger<ProjectFileServer> _logger;
    ...
    public ProjectFileServer(ILogger<ProjectFileServer> logger)
    {
        _logger = logger;
    }
    ...
    /// </summary>
    public void ConfigureEndpoint(WebApplication application)
    {
    ...
    public string ResolveProjectFileUrl(string path, ResourceKey contextResource = default)
    {
