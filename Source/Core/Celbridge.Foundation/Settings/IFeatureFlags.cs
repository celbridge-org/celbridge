...
/// the flags named in FeatureFlagConstants.BuildTimeFlags.
/// </summary>
public interface IFeatureFlags
{
    /// <summary>
    /// Returns true if the specified feature is enabled.
    /// Checks project overrides first, then falls back to application-level configuration. A build-time
    /// flag skips the override and reads the application-level value.
    /// </summary>
    ...
    /// An unset feature defaults to enabled, and an unset build-time flag to disabled.
    /// </summary>
    bool GetApplicationValue(string featureName);

    /// <summary>
    /// Applies project-level feature flag overrides.
    /// These take precedence over application-level settings. An override naming a build-time flag has
    /// no effect.
    /// </summary>
