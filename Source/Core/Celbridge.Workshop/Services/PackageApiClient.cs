...
public class PackageApiClient : IPackageApiClient, IDisposable
{
    ...
    public async Task<Result<RemotePackageDetails>> GetPackageAsync(string packageName)
    {
        ...
        foreach (var versionDetail in details.Versions ?? [])
        {
            workshopVersions.Add(new RemoteWorkshopVersion(
                versionDetail.Version,
                versionDetail.Author ?? string.Empty,
                versionDetail.Date,
                versionDetail.Deleted,
                versionDetail.ContentHash ?? string.Empty,
                versionDetail.Summary ?? string.Empty));
        }
    ...
    public async Task<Result<RemotePublishReceipt>> PublishVersionAsync(string packageName, byte[] zipData, string? summary = null, string? author = null)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            // The error body carries the server's validation message, such as a bad
            // bundle, and never contains credential material.
    ...
    public async Task<Result<byte[]>> DownloadVersionAsync(string packageName, int workshopVersion)
    {
        var sendResult = await _sender.SendAsync(HttpMethod.Get, $"api/packages/{Uri.EscapeDataString(packageName)}/versions/{workshopVersion}/download/");
        if (sendResult.IsFailure)
        {
            return Result.Fail($"Failed to download workshop version {workshopVersion} of package '{packageName}'").WithErrors(sendResult);
        }

        using var response = sendResult.Value;
        if (response.StatusCode == HttpStatusCode.Gone)
        {
            return Result.Fail($"Workshop version {workshopVersion} of package '{packageName}' has been deleted and can no longer be downloaded.");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            return Result.Fail($"Failed to download workshop version {workshopVersion} of package '{packageName}' (HTTP {(int)response.StatusCode})");
        }
    ...
    public async Task<Result<byte[]>> DownloadLatestAsync(string packageName)
    {
        ...
        if (sendResult.IsFailure)
        {
            return Result.Fail($"Failed to download the latest workshop version of package '{packageName}'").WithErrors(sendResult);
        }

        using var response = sendResult.Value;
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            return Result.Fail($"Package '{packageName}' has no live workshop version to download. It may not exist on the workshop, or every workshop version may have been deleted.");
        }

        if (!response.IsSuccessStatusCode)
        {
            return Result.Fail($"Failed to download the latest workshop version of package '{packageName}' (HTTP {(int)response.StatusCode})");
        }
    ...
    public async Task<Result> SetAliasAsync(string packageName, string alias, int workshopVersion)
    {
        var body = JsonSerializer.Serialize(new AliasVersionBodyDto(workshopVersion));
        ...
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            // The server returns 404 both for a missing package and for a workshop version the
            // package does not have. The version target is the more common scripting mistake,
            // so the message names it first.
            return Result.Fail($"Cannot point alias '{alias}' at workshop version {workshopVersion}. Workshop version {workshopVersion} does not exist on package '{packageName}', or the package itself was not found.");
        }

        if (!response.IsSuccessStatusCode)
        {
            return Result.Fail($"Failed to set alias '{alias}' to workshop version {workshopVersion} of package '{packageName}' (HTTP {(int)response.StatusCode})");
        }
    ...
    public async Task<Result> DeleteVersionAsync(string packageName, int workshopVersion)
    {
        using var content = BuildDeleteReasonContent();

        var sendResult = await _sender.SendAsync(HttpMethod.Delete, $"api/packages/{Uri.EscapeDataString(packageName)}/versions/{workshopVersion}/", content);
        if (sendResult.IsFailure)
        {
            return Result.Fail($"Failed to delete workshop version {workshopVersion} of package '{packageName}'").WithErrors(sendResult);
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            return Result.Fail($"Failed to delete workshop version {workshopVersion} of package '{packageName}' (HTTP {(int)response.StatusCode})");
        }
    ...
    public async Task<Result<string>> GetVersionHistoryAsync(string packageName, int workshopVersion)
    {
        var sendResult = await _sender.SendAsync(HttpMethod.Get, $"api/packages/{Uri.EscapeDataString(packageName)}/versions/{workshopVersion}/history/");
        if (sendResult.IsFailure)
        {
        ...
        if (!response.IsSuccessStatusCode)
        {
            return Result.Fail($"Failed to get the history of package '{packageName}' as of workshop version {workshopVersion} (HTTP {(int)response.StatusCode})");
        }
    ...
    {
        RemoteWorkshopVersionSummary? latestWorkshopVersion = null;
        if (entry.LatestVersion is not null)
        {
            latestWorkshopVersion = new RemoteWorkshopVersionSummary(
