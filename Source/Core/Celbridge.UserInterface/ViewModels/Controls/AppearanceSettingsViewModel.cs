...
public partial class AppearanceSettingsViewModel : ObservableObject
{
    ...
    public void OnShown()
    {
        // The View menu can change the theme while this section is showing, so follow it rather than
        // showing the value read at construction. A change made while it was hidden is picked up by the
        // reflect below.
    ...
    // The settings dialog holds the command queue while it is open, so the theme is applied off the queue.
