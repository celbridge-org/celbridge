...
/// <summary>
/// Result returned by workshop_publish_page with the served path and URL the workshop
/// assigned, and the size of the uploaded bundle.
/// </summary>
public record class WorkshopPublishPageResult(string Path, string Url, int Entries, long Size);
...
public partial class WorkshopTools
{
    ...
    [McpServerTool(Name = "workshop_publish_page", Destructive = true)]
    [ToolAlias("workshop.publish_page")]
    [RelatedGuides("pages_overview", "resource_keys", "silent_vs_interactive")]
    public async partial Task<CallToolResult> PublishPage(string resource = "", bool confirmWithUser = true)
    {
        ...
        if (confirmWithUser)
        {
            var confirmed = await ConfirmPublishPageAsync(publishPath);
            if (!confirmed)
            {
        ...
        var result = new WorkshopPublishPageResult(page.Path, page.Url, archive.EntryCount, archive.ZipData.Length);
        var json = JsonSerializer.Serialize(result, JsonOptions);
        return ToolResponse.Success(json);
    }

    private async Task<bool> ConfirmPublishPageAsync(string publishPath)
    {
        var localizerService = GetRequiredService<Celbridge.Localization.ILocalizerService>();
        var title = localizerService.GetString("Workshop_PublishPageConfirm_Title");
        var message = localizerService.GetString("Workshop_PublishPageConfirm_Message", publishPath);
    ...
    // name whichever it has to hand (mirrors workshop_publish_package).
