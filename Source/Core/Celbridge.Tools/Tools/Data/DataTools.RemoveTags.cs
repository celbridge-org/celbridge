...
{
    /// <summary>Atomically remove a batch of tags from a resource's tag list.</summary>
    [McpServerTool(Name = "data_remove_tags", Idempotent = true)]
    [ToolAlias("data.remove_tags")]
    [RelatedGuides("resource_keys")]
    public async partial Task<CallToolResult> RemoveTags(string resource, string tagsJson)
    {
        ...
        var commandResult = await ExecuteCommandAsync<IRemoveTagsCommand>(command =>
        {
