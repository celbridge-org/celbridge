...
Resource key of the file or folder to move. See `resource_keys` for the syntax.
...
- `explorer_copy` — same shape, but leaves the source in place.
- `explorer_rename` — interactive rename via dialog.
- `explorer_undo` — reverse the move.
- `resource_keys`, `undo_semantics`.
