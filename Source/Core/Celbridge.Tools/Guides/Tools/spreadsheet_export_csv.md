...
- With a `destination` resource key, the CSV is written to that file via the audited file-write command and a small JSON metadata object is returned instead. Useful so a large export does not have to round-trip through the agent or script context.
