...
- `utilities` (array) — every available utility. Each entry has:
  ...
  - `area` (string) — the workspace area the utility occupies: `"utility"` when it is a rail surface in the Utility Panel, or a document area token (`"main"`) when it is a document tab. A custom utility can move between the two at runtime; Explorer and Search are always `"utility"`, and the launchers are always `"main"`.
