...
- `utilities` (array) — every available utility. Each entry has:
  ...
  - `isVisible` (bool) — whether the user can currently see the utility. That means it is the selected rail item (in the `utility` area) or the tab its section is showing (in a document area), **and** that its area is on screen. A utility can be the selected rail item while the Utility Panel is collapsed, and that reports `false`, because nothing is on screen. In Focus and Presentation only the area the mode is showing is on screen. Trust this rather than inferring visibility from `currentArea`.
