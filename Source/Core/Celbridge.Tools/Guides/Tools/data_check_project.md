...
- **Orphan sidecars** — `.cel` files with no parent file present on disk.
- **Broken sidecars** — `.cel` files (including invalid `.cel.cel` filenames) that fail to parse.
...
Pure read; the tool does not repair anything. Resolution is the caller's responsibility: rename or restore the missing target, delete or re-parent the orphan, repair or delete the broken sidecar.
