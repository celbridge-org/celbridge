...
Reveals a utility by its id, optionally moving it to a workspace area first. Utilities are the buttons on the Utility Panel rail: the built-in Explorer and Search live in the rail on the left, the built-in Project Settings and Community open a document, and custom utilities live in the rail until the user docks one into a document tab. This one tool reveals a utility wherever it currently is, so you do not need to know where it lives to bring it up.
...
- `utilityId` — the id of the utility to show. Two forms, one scheme:
  - Built-in: `celbridge.explorer`, `celbridge.search`, `celbridge.project-settings` and `celbridge.community`.
