...
- `unhealthyDocuments` (array) — the open documents whose hosted page has stopped responding, omitted entries being healthy. Each entry has `resource`, `wakeFailures` (consecutive keep-alive wakes the page did not answer, macOS only) and `processFailures` (times the process rendering the page died or was replaced). A document listed here may be showing stale or blank content, and its editor state may not be saveable. Both counts are observed at most once every 30 seconds, so a page that has just died can still report as healthy.
