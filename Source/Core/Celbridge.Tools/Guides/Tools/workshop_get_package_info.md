...
- `workshopVersions` (array) — one object per workshop version, newest first, each with:
  - `workshopVersion` (int) — the number the workshop assigned.
  - `author` (string) — who published it.
  - `date` (datetime) — when it was published.
  - `deleted` (bool) — true if the workshop version's content has been removed, so it cannot be installed.
...
- `aliases` (array) — one object per alias, each with `alias` (string) and `workshopVersion` (int). Every alias, such as `stable`, is publisher-defined. `latest` never appears, because it is a reserved selector rather than an alias.

## Gotchas

- A `404` from the workshop (no such package) surfaces as an error. Check the name with `workshop_list_packages`. A package removed with `workshop_unpublish_package` no longer exists, so it fails the same way.
