...
- `packages` — each loaded project package: `name`, `version` (read from the package's `HISTORY.md`; `null` when the package was authored in place, so no history file exists), and `folder` (the resource key of the package folder, e.g. `project:packages/my-widget`).
...
- **`version` is read from `HISTORY.md`**, not from a manifest field — a package manifest carries no version. A `null` version is normal for a package authored in place.
