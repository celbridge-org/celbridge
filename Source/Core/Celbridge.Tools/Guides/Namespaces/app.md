...
- **Call `app_get_state` first.** It reports whether a project is loaded, the feature-flag map (consult before invoking a feature-gated tool such as `webview_eval`), the focused panel, the layout-mode visibility flags, and the installed Python package list. The response also names the orientation guide via `agentDocs.entry`.
...
- `app_get_state` — workspace state snapshot (project load, feature flags, focused panel, layout, Python packages).
- `app_get_version` — application version.
...
- `agent_instructions` — the orientation guide named by `app_get_state.agentDocs.entry`.
- `workspace_panels` — what `focusedPanel` and `layoutMode` mean.
- `silent_vs_interactive` — which tools block on user input.
- `command_conventions` — synchronous command semantics shared by every tool.
