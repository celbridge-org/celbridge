...
- **Conditional-formatting type names are arity-suffixed.** Use `colorScale2` (`minColor` + `maxColor`) or `colorScale3` (`minColor` + `midColor` + `maxColor`) — there is no plain `colorScale`. See `spreadsheet_set_conditional_formatting`.
