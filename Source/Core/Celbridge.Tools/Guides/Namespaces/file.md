...
- **Edits write straight to disk.** `file_write`, `file_edit`, `file_multi_edit`, `file_find_replace`, `file_apply_edits`, `file_delete_lines`, and `file_write_binary` save immediately. If the document is open, the editor reloads from disk and Monaco's undo history is wiped. The agent's edit cannot be reverted with Ctrl+Z. See `file_changes`.
...
1. Need regex (capture groups, character classes, alternation) or a line-range scope? Use `file_find_replace`.
...
- `file_find_replace` — regex or literal pattern replace, optional line-range scope.
- `file_write` — replace a file's contents or create a new file.
- `file_write_binary` — write raw bytes from base64.
- `file_apply_edits` — coordinate-based batch edits at known line/column positions.
