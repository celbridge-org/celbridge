...
- **You don't need to poll.** A `file_write` followed by `file_read` always sees the new contents. A `document_open` followed by `document_get_state` always shows the new tab.
