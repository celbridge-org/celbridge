...
- **Document shortcuts**: a `[[shortcut]]` entry adds a button to the Utility Rail that opens `resource` as a document, in whichever editor that file's extension resolves to. Entry order is rail order, and the entries sit ahead of the built-in Project Settings and Workshop buttons. `area` (`main`, `bottom` or `side`, default `main`) picks where the document lands, and applies only while it is not already open. An entry whose `resource` is not a resource key contributes no button.
