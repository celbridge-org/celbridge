...
[[shortcut]]                                                   # a Utility Rail button that opens a file
resource = "readme.md"
icon     = "bs-book"                                           # optional; defaults to a document icon
...
- **Document shortcuts**: a `[[shortcut]]` entry adds a button to the Utility Rail that opens `resource` as a document, in whichever editor that file's extension resolves to. Entry order is rail order, and the entries sit with the utilities the project's packages contribute, ahead of the gap that pins the built-in Project Settings and Community buttons at the end of the rail. `area` (`main`, `bottom` or `side`, default `main`) picks where the document lands, and applies only while it is not already open. An entry whose `resource` is not a resource key contributes no button.
