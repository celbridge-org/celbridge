...
- **Case-insensitive matching** — for `file_grep` use the `(?i)` inline flag or the tool's case-insensitivity option.
...
A pathological pattern can cause exponential backtracking on a hostile input. Tools wrap regex execution in a short timeout and surface a timeout error rather than hanging. For very large inputs, prefer a substring or a fixed-length pattern over heavy alternation.
