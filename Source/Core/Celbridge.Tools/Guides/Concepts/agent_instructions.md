...
- **WebView tools depend on which editor opened the document.** A `.html` page can be inspected in the HTML editor, but a `.html` file opened in the general code editor shows only its source. Check `editorId` from `document_get_state` first.
