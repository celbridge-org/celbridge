...
- **Calling a tool the host withholds** (the `webview.*` namespace and the workshop tools) throws `TypeError: Cannot read properties of undefined`, because the proxy is built from the tools the host returned.
