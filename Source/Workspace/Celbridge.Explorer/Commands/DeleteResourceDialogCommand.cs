...
public class DeleteResourceDialogCommand : CommandBase, IDeleteResourceDialogCommand
{
    ...
    private async Task<Result> ShowDeleteResourceDialogAsync()
    {
        ...
        // Pre-check the policy before confirming: a reserved resource, or one in a
        // read-only location, cannot be deleted, so explain why rather than asking
        // the user to confirm a delete the operation layer would then refuse. The
        // user just triggered the delete, so the message names only the blocked
        // file (or the count, for a multi-selection) and the reason.
        ...
        if (blockedResources.Count > 0)
        {
            ...
            string messageText = blockedResources.Count == 1
                ...
                : _stringLocalizer.GetString("Policy_Blocked_Multiple", blockedResources.Count);
