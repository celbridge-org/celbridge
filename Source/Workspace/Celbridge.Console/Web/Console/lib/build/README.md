...
`.csproj` (`Content Remove="Web\**\lib\build\**"`) — only the copied dist files
in `lib/` ship with the app. `node_modules/` is gitignored.

## Updating xterm.js

1. `cd` into this directory (`Web/Console/lib/build/`)
