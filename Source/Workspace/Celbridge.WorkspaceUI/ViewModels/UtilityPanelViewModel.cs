...
public partial class UtilityPanelViewModel : ObservableObject
{
    ...
    // True from a selection until focus lands on the selected utility. While it is true the accent is shown
    // optimistically and focus reports for other panels are ignored, which suppresses the transient bounce as
    // the outgoing panel is collapsed (WinUI briefly relocates focus off it before the new one receives it).
    private bool _awaitingSelectionFocus;

    /// <summary>
    /// The rail items in display order: built-in utilities first, then custom utilities.
    /// </summary>
    public IReadOnlyList<UtilityItemViewModel> Items => _items;

    /// <summary>
    /// The utility id of the currently selected rail item, or Empty when none is selected.
    /// </summary>
    public EditorId SelectedUtilityId => _selectedUtilityId;

    /// <summary>
    /// Appends a rail item and returns its view model. focusIdentity is the workspace panel this utility
    /// reports focus as (FocusPanelId.CustomUtility for every custom utility).
    /// </summary>
    public UtilityItemViewModel AddItem(EditorId id, FocusPanelId focusIdentity)
    {
    ...
    /// <summary>
    /// Makes the utility with the given id the one the panel is showing. Pass awaitFocus when the utility is
    ...
    /// a different panel is ignored (the transient switch bounce). A report for the selected utility settles
    /// the wait.
    /// </summary>
    public void ReconcileFocus(FocusPanelId focusedPanel)
    {
    ...
    private FocusPanelId SelectedFocusIdentity
    {
        get
        {
            var selectedItem = FindItem(_selectedUtilityId);
            return selectedItem?.FocusIdentity ?? FocusPanelId.None;
        }
    }
    ...
    private bool SelectedUtilityHasFocus
    {
        get
        {
            if (_awaitingSelectionFocus)
            ...
            return selectedFocusIdentity != FocusPanelId.None
    ...
    {
        var utilityHasFocus = SelectedUtilityHasFocus;
        ...
        foreach (var item in _items)
        {
            ...
            item.IsFocused = item.IsSelected && utilityHasFocus;
        }
    }
}
