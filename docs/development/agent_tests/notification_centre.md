...
| An event and the load's condition pending, the list reaching over a web document or the console | click the event's dismiss button | the event leaves the list, the condition stays, and the surface underneath neither receives the click nor takes focus | 1 |
| Several notifications pending, the list reaching over the console | click a row's action | the document the action names opens, the list closes, the notification is still pending, and the console takes no focus | 2 |
| No condition pending, only events | click Clear All | the list closes before the badge disappears, and nothing is left open over the documents | 2 |
...
| The badge collapsed | raise a notification | the badge appears with a count of one and flashes once, and its tooltip reads the notification's line | 2 |
| The badge showing | raise several different notifications in quick succession | one flash, and the count rises by the number raised | 2 |
