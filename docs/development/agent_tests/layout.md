...
| Bottom collapsed, with a shortcut into Bottom whose document is closed | click the shortcut's button | Bottom shows with the document active, and the keyboard is in the document: text typed next lands in it | 2 |
...
keyboard is. Show it by typing and reading the document's text back: the focused panel `app_get_state`
reports can name Documents while no key reaches the editor.
