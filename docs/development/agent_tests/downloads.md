...
| Finished downloads, one of which failed, the list open | click the failed row's remove button, then each remaining row's | the failed row leaves the list and the others stay, the badge loses the error color and goes on counting only what arrived, and removing the last row closes the list and the badge goes, with every downloaded file still in `downloads/` | 2 |
