...
7. The startup lines are written once the shell's output has been quiet for 150 ms, capped at 1.5 s. The injected line clears the screen and, except under cmd, emits the shell family's ready marker before running the command. Output before the marker is discarded, so the buffer starts on a clean screen. A plain shell that launched without a view size holds its lines until the first real size, so its prompt is not drawn at the fallback width.
